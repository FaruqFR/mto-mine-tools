(function(){
  ...
  function labels(){
    ...
    if(haul)haul.innerHTML='<div>Q =</div><div style="display:inline-block;text-align:center;line-height:1.25"><span>60</span><hr style="margin:3px 0"><span>CTa</span></div><span> × q × FF × SF × E</span><small style="display:block;margin-top:8px">CTa dalam menit</small>';
    ...
    // Produktivitas alat angkut mengikuti rumus tanpa jumlah pass (n).
  ...
  window.truckProduction=function(){const c=n('excCTProd'),q=n('bucketCap'),FF=n('fillFactor'),SF=n('swellFactor'),E=n('effTruck')/100;const v=c>0?(60/c)*q*FF*SF*E:NaN;if($('truckProdOutput'))$('truckProdOutput').textContent=Number.isFinite(v)?fmt(v):'—'};
  ...
  ['input','change'].forEach(ev=>document.addEventListener(ev,e=>{if(e.target?.id==='mfCta')window.matchFactor();if(['excCTProd','bucketCap','fillFactor','swellFactor','effTruck'].includes(e.target?.id))window.truckProduction();if(['truckCTSec','bucketCap2','fillFactor2','swellFactor','effExc'].includes(e.target?.id))window.excavatorProduction();},true));
})();