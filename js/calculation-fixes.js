(function(){
  ...
  function labels(){
    ...
    set('passes','n - Jumlah Pass Pengisian <small>(pass)</small>');
  ...
  ['input','change'].forEach(ev=>document.addEventListener(ev,e=>{if(['mfCta','loadingTimeMF','passes'].includes(e.target?.id))window.matchFactor();if(['excCTProd','bucketCap','fillFactor','swellFactor','effTruck'].includes(e.target?.id))window.truckProduction();if(['truckCTSec','bucketCap2','fillFactor2','swellFactor','effExc'].includes(e.target?.id))window.excavatorProduction();},true));
})();